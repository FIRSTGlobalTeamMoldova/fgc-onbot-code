...
//@Config
public class HDriveComponentV5 extends Component {
    ...
    public void runLoop() {
        ...
        double relativeHeading = imu.getRelativeHeading();

        double turnVal = 1;
        if (Math.abs(driverGamepad.getRightX()) > 0.1) {
            currentOrientation = relativeHeading;
            turnVal = 0;
        ...
        telemetry.addData("Slow mode coef", slowModeCoef);
        telemetry.addData("Slow mode turn coef", slowModeTurnCoef);
        ...
        if (hDriveToggle.isDown()) {
            ...
            if (leftTrigger > 0.1 || rightTrigger > 0.1) {
                ...
                if (!gyroState) {
                    double vel = rotationController.calculate(getDegreesToTarget(currentOrientation, relativeHeading));
                    telemetry.addData("Trigger tan vel", vel);
                ...
                if (!gyroState) {
                    double vel = holoRotationController.calculate(getDegreesToTarget(currentOrientation, relativeHeading));
                    telemetry.addData("Joystick tan vel", vel);
